...
econtrast6 <- function(X1,X2){		# measure="HR"
  ...
  for(i in 2:p){
    ...
    y[,(i-1)] <- log(-log(1-pr1)) - log(-log(1-pr2))
  ...
  for(i in 2:(p-1)){
    ...
    s11 <- (((pr1-1)*log(1-pr1))^-2)*(pr1*(1-pr1)/m1) + (((pr2-1)*log(1-pr2))^-2)*(pr2*(1-pr2)/m2)
    ...
    for(j in (i+1):p){
      ...
      s11 <- (((pr2-1)*log(1-pr2))^-2)*(pr2*(1-pr2)/m2) + (x1 - x1) + (w1 - w1)	
      S <- cbind(S, s11)
